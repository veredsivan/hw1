...
//creats new stack
void* stack_create(int max_num_of_elem,
					clone_t student_clone,
					destroy_t student_destroy, 
					print_t student_print)
{
	stack *pstack_t = (void*)calloc(max_num_of_elem, sizeof(char));
	return pstack_t;
}

//destroys struck 
enum Result stack_destroy(pstack_t pstack)
{
	if (pstack == NULL)
		return FAIL;
	free(pstack);
...
enum RESULT stack_push(pstack_t stack, elem_t new_elem)
{
